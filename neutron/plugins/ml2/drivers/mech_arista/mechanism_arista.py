...
from neutron.plugins.ml2.drivers.mech_arista import config
...
class AristaRPCWrapper(object):
    ...
    def __init__(self):
        ...
        self._register_with_eos()

    def _keystone_url(self):
        keystone_auth_url = '%s://%s:%s/v2.0/' % (self.keystone_conf.auth_protocol,
    ...
    def plug_host_into_vlan(self, vm_id, host, port_id, 
        ...
        if port_name: 
            ...
            cmds.append('port id %s network-id %s' % \
                        (port_id, network_id))
        cmds.append('exit') # exit for vm mode
        cmds.append('exit') # exit for tenant mode
        self._run_openstack_cmd(cmds)

    def unplug_host_from_vlan(self, vm_id, host, port_id,
    ...
    def _run_openstack_cmd(self, commands, deleteRegion=None):
        ...
        command_end = ['exit', 'exit'] #exit for region and management each.
            ...
            for vm_id in db_vm_list:
                if vm_id not in eos_vm_list: 
                    ...
                    for port in ports:
                        ...
                        port_name = port['name']
                        self._rpc.plug_host_into_vlan(vm['vmId'],
                                                      vm['host'],
                                                      port_id,
                                                      network_id,
                                                      tenant,
...
class AristaDriver(driver_api.MechanismDriver):
    ...
    def create_network_precommit(self, context):
        ...
        with self.eos_sync_lock:
            ...
            self.net_storage.remember_network(tenant_id, network_id, segmentation_id)
    ...
    def create_network_postcommit(self, context):
        ...
        with self.eos_sync_lock:
            if self.net_storage.is_network_provisioned(tenant_id, network_id):
                try:
                    self.rpc.create_network(tenant_id, network_id, network_name, vlan_id)
                except arista_exc.AristaRpcError:
    ...
    def update_network_precommit(self, context):
        ...
        if new_network['name'] != orig_network['name']:
            msg = _('Network name changed from %s to ' '%s' % \
                    (orig_network['name'], new_network['name'])) 
            LOG.info(msg)
        ...
        if host and is_vm_boot:
            ...
            try:
                with self.eos_sync_lock:
                    ...
                    segmentation_id = s.get_segmentation_id(tenant_id, network_id)
                    vm_provisioned = s.is_vm_provisioned(device_id, host, port_id,
                                                         network_id, tenant_id)
                    net_provisioned = s.is_network_provisioned(tenant_id, network_id,
                                                               segmentation_id)
                    if vm_provisioned and net_provisioned:
                        self.rpc.plug_host_into_vlan(device_id,
                                                     hostname,
                                                     port_id,
                                                     network_id,
                                                     tenant_id,
    ...
    def delete_port_postcommit(self, context):
        ...
        try:
            with self.eos_sync_lock:
                s = self.net_storage
                hostname = self._host_name(host)
                self.rpc.unplug_host_from_vlan(device_id,
                                               hostname,
                                               port_id,
                                               network_id,
                                               tenant_id)
